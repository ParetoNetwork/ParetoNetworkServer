    ...
    if (typeof provider !== "undefined") {
      web3.eth.getAccounts(async (err, accounts) => {
        ...
        let gasApprove = await ParetoTokenInstance.methods
          .approve(Intel.options.address, depositAmount*5)
        ...
        // const gasSendReward = await Intel.methods.sendReward(content.ID, depositAmount*3).estimateGas({ from: provider_address });
        // await Intel.methods.sendReward(content.ID, depositAmount*3).send({ from: provider_address, gas: gasSendReward });
