...
export default class dashboardService {
  ...
  static getIntel(intel, onSuccess, onError) {
    return http.get('/v1/content/?compact=true' + intel).then(res => {
      if (res.data.success) {
  ...
  static getContent(params, onSuccess, onError) {
    return http.get('/v1/content/me?compact=true', {params}).then(res => {
      if (res.data.success) {
        return onSuccess(res.data.data);
      } else {
  ...
  static getAllContent(params, onSuccess, onError) {
    ...
    return http.get('/v1/content?compact=true&limit=' + limit + '&page=' + page + total_query).then(res => {
      //console.log(res);
      if (res.data.success) {
        return onSuccess(res.data.data);
      } else {
