                        ...
                        <b-form-radio-group v-model="selectedPath">
                                    ...
                                    <b-dropdown variant="link" size="lg" no-caret>
                                        <template slot="text">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span> {{ parseInt(paths[0].selected.tokens || '0')}} PARETO <i class="fa fa-sort-down"></i> </span>
                                            </div>
                                        </template>
                                        <b-dropdown-item v-for="user in paths[0].address" @click="onAddresSelected('s44\'/60\'/0\'/0/0', user, 0)">
                                            <div class="d-flex justify-content-between">
                                    ...
                                    <b-dropdown variant="link" size="lg" no-caret>
                                        <template slot="text">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span> {{ parseInt(paths[1].selected.tokens || '0')}} PARETO <i class="fa fa-sort-down"></i> </span>
                                            </div>
                                        </template>
                                        <b-dropdown-item v-for="user in paths[1].address" @click="onAddresSelected('l44\'/60\'/0\'/0', user, 1)">
                                            <div class="d-flex justify-content-between">
                                                <span class="span-ellipsis"> {{substringAddress(user.address)}} </span>
                                                <span> {{parseInt(user.tokens || '0')}} PARETO</span>
                                            </div>
                            ...
                            <b-row class="m-2 mt-4">
                                <div class="col-12 col-md-3 mb-2 p-0">
                                    <b-form-radio :disabled="!supported" v-on:change="onAddresSelected(customPath, paths[2].address[0] || '', 2)"
                                    ...
                                    <b-dropdown variant="link" size="lg" no-caret :disabled="!paths[2].address">
                                        <template slot="text">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <span> {{ parseInt(paths[2].selected.tokens || '0')}} PARETO <i class="fa fa-sort-down"></i> </span>
                                            </div>
                                        </template>
                                        <b-dropdown-item  v-for="user in paths[2].address" @click="onAddresSelected(customPath, user, 2)">
                                            <div class="d-flex justify-content-between">
                                                <span class="span-ellipsis"> {{substringAddress(user.address)}} </span>
                                                <span> {{parseInt(user.tokens || '0')}} PARETO</span>
                                            </div>
        ...
        methods: {
                ...
                this.paths.forEach(path => {
                    ...
                    authService.getWalletAccounts(myPath, page, limit, data => {
                        ...
                        authService.getTokens( addressList , listData =>{
                                ...
                                /*
                                path.address = userList.map( user => {
                                    return {
                                        address : user.address,
                                        tokens : user.tokens
                                    }
            ...
            scrollAddressList: function(index, path){
                ...
                if(path.scrollTop + path.offsetHeight >= path.scrollHeight) {
                    ...
                    authService.getWalletAccounts(this.selectedPath, page, 10, data => {
                        const list = Object.values(data);
