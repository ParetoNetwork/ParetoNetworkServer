    ...
    intelController.getAllAvailableContent = async function (req, callback) {
        ...
        await  intelController.getQueryContentByUser(req.user, null, async function (error, contentDelay, queryFind, percentile) {
            ...
            try {
                ...
                const allResults = await ParetoContent.find(queryFind).sort({dateCreated: -1}).skip(page * limit).limit(limit).populate([{ path: 'assets.asset', select:'symbol name -_id'},
    ...
    intelController.getContentByIntel = function (req, intel, callback) {
        intelController.getQueryContentByUser(req.user, intel, async function (error, contentDelay, queryFind, percentile) {
            ...
            try {
                ...
                const allResults = await ParetoContent.find(queryFind).sort({dateCreated: -1}).populate([{ path: 'assets.asset', select:'symbol name -_id'},
                  {path: 'createdBy' , select: 'address alias aliasSlug biography profilePic -_id'},
                  {path: 'rewardsTransactions', select: 'amount sender', populate:{ path: 'profile', select: 'address alias aliasSlug -_id'} }]).exec();
