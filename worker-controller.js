...
workerController.aproxAllScoreRanking = async function (callback) {
  web3.eth.getBlock('latest')
    .then(function (res) {
      ...
      ParetoAddress.find({
        tokens: {$gt: 0},
        block: {$gt: CONTRACT_CREATION_BLOCK_INT}
      }, 'address score tokens block bonus', function (err, results) {
        if (err) {
          callback(null, {});
        } else {
          const bulkop = [];
          ...
          if (data.status == 0) {
            web3.eth.getTransactionReceipt(data.txHash, function (err, receipt) {
              if (receipt) {
                ParetoTransaction.findOneAndUpdate({
                  txHash: data.txHash,
                  status: 0
                }, {status: (data.event == 'distribute') ? 3 : 1}, {multi: false}, function (err, r) {
                  if (data.event != 'distribute' && r) {
                    ...
                    if (data.event == 'distribute' && r) {
                      ParetoContent.findOneAndUpdate({id: r.intel}, {
                        distributed: true,
                        txHashDistribute: data.txHash
                      }, function (err, r) {
                      })
                    }
            ...
            if (data.txRewardHash && data.status == 2) {
              web3.eth.getTransactionReceipt(data.txRewardHash, function (err, receipt) {
                if (receipt) {
                  ParetoTransaction.findOneAndUpdate({
                    txRewardHash: data.txRewardHash,
                    status: 2
                  }, {status: 3}, {multi: false}, function (err, data) {
                  });
                }
