...
app.get("/getIntels", (req, res) => {
  ...
  app.get("/getContributors/:intelId", (req, res) => {
    ...
    app.post('/v1/config_basic', function (req, res) {
        ...
        res.status(200).json(ErrorHandler.getSuccess({ netWorkId: netWorkId , etherscan: process.env.ETHERSCAN_URL, intelAddress: intel.networks[netWorkId].address, paretoAddress: pcontract, psignversion: psignversion}));
    });
    ...
    app.post('/v1/config', function (req, res) {
        ...
        const pcontract = process.env.CRED_PARETOCONTRACT;
        const psignversion = process.env.PARETO_SIGN_VERSION;
        res.status(200).json(ErrorHandler.getSuccess({intel: intel.abi, pareto: pareto.abi, netWorkId: netWorkId, etherscan: process.env.ETHERSCAN_URL , intelAddress: intel.networks[netWorkId].address, paretoAddress: pcontract, psignversion: psignversion}));
    });
