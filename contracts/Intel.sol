...
contract Intel{
    ...
    function sendReward(uint intelIndex, uint rewardAmount) public returns(bool success){
        ...
        token.transferFrom(msg.sender, this, rewardAmount);  // transfer token from caller to Intel contract
    ...
    function distributeReward(uint intelIndex) public returns(bool success){
        ...
        if (intel.balance > intel.desiredReward){         // check if the Intel's balance is greater than the reward desired by Provider
            ...
            token.transfer(intel.intelProvider, intel.desiredReward);    // tarnsfer tokens to the provider's address equal to the desired reward

        } else {
            distributed_amount = intel.balance;
            token.transfer(intel.intelProvider, intel.balance);  // transfer token to the provider's address equal to Intel's balance
        }
